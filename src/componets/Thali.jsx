...
const Thali = () => {
    ...
    const thalis = foodItems?.filter((c)=>c.category ==="thali")
   
//for update cart value
const addToCart = (thaliDetail)=>{
  if(!cartItems.find((added)=>added.id === thaliDetail.id)){
    dispatch({
      type:ACTION_TYPE.SET_CART_ITEMS,
      cartItems:[...cartItems,thaliDetail]
    });
    setThaliList(cartItems)
  }else{
    alert("🥘This Thali Already Added🥘")
  }
  ...
  return (
    <div className='w-full my-6'>
            ...
            before:w-16 before:h-1 before:-bottom-2 before:left-0  before:bg-gradient-to-tr
             from-orange-200 to-orange-600 before:content  '>Thali</p>
        ...
        className='w-full my-12 overflow-x-scroll scrollbar-none  
            ...
            {
                thalis && thalis.map((thaliDetail)=>
                
                <div key={thaliDetail.id}
            ...
            border-gray-200 rounded-md flex justify-center items-center flex-col '>
                    ...
                    src={thaliDetail.imageURL}
                     alt={thaliDetail.title} 
                    
                     />
                    </div>
                    <div className='w-full flex justify-center items-center flex-col'>
                    <p className='text-lg font-semibold my-3'>{thaliDetail.title}</p>
                    <p className='text-base font-semibold my-3'>{thaliDetail.description}</p>
                    
                    </div>
                    <div className='w-full flex justify-between'>
                    <p className='text-lg font-semibold my-3 flex items-center'>
                    <FaRupeeSign/>{thaliDetail.price}</p>
                    <motion.button 
                     whileTap={{scale:0.75}}
                    className=' bg-gradient-to-br from-orange-600 to-orange-300 text-white px-10  
                    rounded-md font-semibold'
                    onClick={()=>addToCart (thaliDetail)}
                    >ADD</motion.button>
                    </div>
            </div>
